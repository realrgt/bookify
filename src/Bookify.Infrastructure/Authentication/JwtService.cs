...
internal sealed class JwtService : IJwtService
{
    ...
    public async Task<Result<string>> GetAccessTokenAsync(string email, string password, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var authResponse = await _httpClient.PostAsync("", authRequestContent, cancellationToken);
            authResponse.EnsureSuccessStatusCode();
            var authToken = await authResponse.Content.ReadFromJsonAsync<AuthorizationToken>();
